...
def type_2_buy_sell_normal (target_dic) :
    ...
    if target_dic['bought_state'] == 1 :   # 매수가 되어 있는 상태라면
        ...
        print('Cheking Sell condition :', target_coin)
        ...
        if (time_elapse_bought > 1) and (DF_check_peak['volume'][-2] < (target_dic['sell_method_vol_cri'] * DF_check_peak.iloc[(-5) : -2]['volume'].mean())) and         ((DF_check_peak['ema_short'][-1] / DF_check_peak.loc[DF_check_peak.iloc[-(time_elapse_bought + 1) : -1]['high'].idxmax()]['high']) < target_dic['ratio_peak_diff']) :
        ...
        elif (time_elapse_bought > 3) and         (DF_check_peak.loc[DF_check_peak.iloc[-13 : -1]['ratio_prior_to_cur'].idxmax()]['ratio_prior_to_cur'] < target_dic['ratio_open_check']) and         (DF_check_peak.iloc[-13 : -1]['open'].std() < (target_dic['ratio_reduced'] * (DF_check_peak.iloc[-(time_elapse_bought + 3 + 1) : -(time_elapse_bought - 3 + 1)]['open'].std()))) and         (DF_check_peak.iloc[-7 : -1]['volume'].mean() < (target_dic['ratio_reduced'] * (DF_check_peak.iloc[-(time_elapse_bought + 3 + 1) : -(time_elapse_bought - 3 + 1)]['volume'].mean()))) :
            
            transaction_sell = upbit.sell_market_order(target_coin, get_balance(target_coin[4:]))  # 시장가에 매도
            time.sleep(5)
            print('\nnow :', (datetime.datetime.now() + datetime.timedelta(seconds=(time_factor * 3600))))
            print('sell_transaction_result_by_sell_ratio_trend :', transaction_sell)
        ...
        elif (time_elapse_bought > 24) and         (DF_check_peak.loc[DF_check_peak.iloc[-13 : -1]['ratio_prior_to_cur'].idxmax()]['ratio_prior_to_cur'] < target_dic['ratio_open_check']) and         ((DF_check_peak['ratio_prior_to_cur'][-13 : -1].mean() < target_dic['ratio_prior_to_cur_long']) or          (DF_check_peak.iloc[-25 : -1]['gap_prior_to_1'].mean() < (target_dic['ratio_mean_long'] * (DF_check_peak.iloc[(50 * 24):]['gap_prior_to_1'].mean())))) :
        ...
        elif (time_elapse_bought > 1) and         (DF_check_peak.iloc[-(target_dic['continuous_fall'] + 1) : ]['fall_check'].sum() >= target_dic['continuous_fall']) and (DF_check_peak['fall_check'][-1] == 1) and         ((DF_check_peak['low'][-1] / DF_check_peak['open'][-(target_dic['continuous_fall'] + 1)]) < (1 - (target_dic['continuous_fall_ratio'] * target_dic['ratio_sell_forced']))) :
